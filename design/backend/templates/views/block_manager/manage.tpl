    ...
    <div class="sidebar-row layouts">
        ...
        <ul class="unstyled">
            {hook name="layouts:layout_page_sidebar"}
                <li>{__("dispatch")}: <span class="pull-right">{$location.dispatch}</span></li>
                <li>{__("name")}: <span class="pull-right">{$location.name}</span></li>
            {/hook}
        </ul>
        <div class="sidebar-field text-right">
            <a class="cm-dialog-opener right" href="{"block_manager.update_location?location=`$location.location_id`&s_layout=`$location.layout_id`"|fn_url}">{__("layout_meta_data")}</a>
        </div>
    </div>
    <hr>
    {/hook}
