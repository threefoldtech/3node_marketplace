{if !$chains}
    <span class="buy-together-info-message">
        ...
        {else}
            {if !$product_data.company_id}
                {__("master_products.buy_together_info_message_for_vendor", ["[href]" => "products.sell_master_product?master_product_id=`$product_data.product_id`&selected_section=buy_together"|fn_url])}
            {/if}
        {/if}
    </span>
{/if}