...
{/if}
        ...
        <div id="{$shipping_estimation_block_id}">
            ...
            <form class="cm-ajax" name="{$prefix}estimation_form{$id_suffix}" action="{""|fn_url}" method="post">
                ...
                <input type="hidden" name="result_ids" value="{$shipping_estimation_block_id},shipping_estimation_buttons{$id_suffix},shipping_estimation_rates{$id_suffix}" />
                ...
                <div class="{$buttons_class}">
                    {include file="buttons/button.tpl"
                             but_text=__("get_rates")
                             but_name="dispatch[checkout.shipping_estimation.get_rates]"
                             but_role="text"
                             but_id="but_get_rates{$id_suffix}"
                    }
                </div>
            ...
            {if $runtime.mode == "shipping_estimation" || $smarty.request.show_shippings == "Y"}
                {if !$cart.shipping_failed && !$cart.company_shipping_failed}
                    ...
                    <input type="hidden" name="result_ids" value="checkout_totals_*" />
                    ...
                    <div class="{$buttons_class}">
                        {include file="buttons/button.tpl"
                                 but_text=__("select")
                                 but_role="text"
                                 but_name="dispatch[checkout.update_shipping]"
                                 but_id="but_select_shipping{$id_suffix}"
...
{if $location == "popup"}
<div class="ty-estimation-buttons buttons-container" id="shipping_estimation_buttons{$id_suffix}">
    {if $runtime.mode == "shipping_estimation" || $smarty.request.show_shippings == "Y"}
        {include file="buttons/button.tpl"
                 but_text=__("recalculate_rates")
                 but_external_click_id="but_get_rates{$id_suffix}"
                 but_role="text"
                 but_meta="ty-btn__secondary cm-external-click ty-float-right ty-estimation-buttons__rate"
        }

        {include file="buttons/button.tpl"
                 but_text=__("select_shipping_method")
                 but_external_click_id="but_select_shipping{$id_suffix}"
                 but_meta="ty-btn__secondary cm-external-click cm-dialog-closer"
        }
    {else}
        {include file="buttons/button.tpl"
                 but_text=__("get_rates")
                 but_external_click_id="but_get_rates{$id_suffix}"
                 but_meta="ty-btn__secondary cm-external-click"
        }
    {/if}
<!--shipping_estimation_buttons{$id_suffix}--></div>
{/if}
