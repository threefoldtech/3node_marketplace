...
function fn_vendor_categories_fee_vendor_plans_calculate_commission_for_payout_post($order_info, $company_data, &$payout_data)
{
    ...
    $exclude_taxes_from_calculation = !YesNo::toBool($vendor_plans_settings['include_taxes_in_commission']);
    ...
    $tax_amount_included_to_surcharge = fn_vendor_plans_get_tax_amount($order_info['taxes'], TaxApplies::PAYMENT_SURCHARGE);
    ...
    if ($order_info['total'] > 0) {
        if ($tax_amount_included_to_surcharge < 0) {
            $total = $order_info['total'] - $order_info['payment_surcharge'] + $tax_amount_included_to_surcharge;
        } else {
            $total = $order_info['total'] - $order_info['payment_surcharge'];
        }
        if ($exclude_taxes_from_calculation) {
            $order_tax_amount = array_sum(array_column($order_info['taxes'], 'tax_subtotal'))
                - abs($tax_amount_included_to_surcharge);
            $total -= $order_tax_amount;
            $shipping_cost -= abs($tax_amount_included_to_shipping);
        }
        $total -= $shipping_cost;
    } else {
        $total = 0;
