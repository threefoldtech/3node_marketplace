...
(function (_, $) {
    ...
    for (var id in data) {
      ...
      if (data[id].option && data[id].option.path) {
        ...
        for (var option_id in data[id].option.path) {
          ...
          modifier = parseFloat($(('#pb_option_modifier_' + option_id + '_' + variant_id).split(' ', '')).val());
  ...
  $(_.doc).on('change', '[name="product_bundle_feature_variation_front"]', function (e) {
    ...
    if ($option.length) {
      ...
      $("[name=\"product_data[".concat(old_product_id, "][product_id]\"]")).val($option.data('caProductId'));
    }
  });
})(Tygh, Tygh.$);